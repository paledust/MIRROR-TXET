Shader "GUI/Mirror Double Text"
{
	Properties
	{
        _MainTex ("Font Texture", 2D) = "white" {} 
        _Color ("Text Color", Color) = (1,1,1,1) 
		_DoubleColor("Double Text Color", Color) = (1,1,1,1)
		_Off("Offset", float) = 0
	}
    SubShader { 
		...
		Pass
		{
			...
			return frac(sin(dot(st,fixed2(14.133,77.)))*312554.);
			}
			float random(float y){
				return random(fixed2(y,y));
			}
			sampler2D _MainTex;
			float4 _MainTex_ST;
			float4 _DoubleColor;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				// apply fog
				UNITY_APPLY_FOG(i.fogCoord, col);
				col.rgb = _DoubleColor.rgb;
				col.a *= _DoubleColor.a * random(_Time);
				return col;
			}
			ENDCG
		}
